...
namespace Cango :: inline TaskDesign
{
	...
	public:
		...
		explicit Credential(std::weak_ptr<T> pointer) noexcept;
		...
		ObjectUser<T> AcquireUser() const noexcept;
	};
	...
	public:
		...
		/// @brief 复制一个使用者
		ObjectUser(const ObjectUser& other) noexcept;
	...
	public:
		...
		/// @brief 构造并占有对象
		explicit ObjectOwner(auto&&... args) noexcept;
	...
	template <typename T>
	Credential<T>::Credential() noexcept = default;

	template <typename T>
	Credential<T>::Credential(std::weak_ptr<T> pointer) noexcept : WeakPointer(pointer) {}

	template <typename T>
	Credential<T>::Credential(const Credential& other) noexcept = default;

	template <typename T>
	bool Credential<T>::IsValid() const noexcept { return !WeakPointer.expired(); }
	...
	template <typename T>
	ObjectUser<T> Credential<T>::AcquireUser() const noexcept
	{
		return ObjectUser<T>{WeakPointer.lock()};
	}

	template <typename T>
	ObjectUser<T>::ObjectUser() = default;

	template <typename T>
	ObjectUser<T>::ObjectUser(const ObjectUser& other) noexcept : Pointer(other.Pointer) {}

	template <typename T>
	ObjectUser<T>::ObjectUser(std::shared_ptr<T> other) noexcept : Pointer(std::move(other)) {}

	template <typename T>
	bool ObjectUser<T>::IsValid() const noexcept { return Pointer != nullptr; }
	...
	template <typename T>
	T& ObjectUser<T>::operator*() const noexcept { return *Pointer; }

	template <typename T>
	T* ObjectUser<T>::operator->() const noexcept { return Pointer.get(); }

	template <typename T>
	ObjectOwner<T>::ObjectOwner(auto&&... args) noexcept : Pointer(
	...
	template <typename T>
	ObjectOwner<T>::ObjectOwner(ObjectOwner&& other) noexcept = default;

	template <typename T>
	ObjectUser<T> ObjectOwner<T>::Use() const noexcept { return ObjectUser<T>{*Pointer}; }

	template <typename T>
	bool ObjectOwner<T>::IsValid() const noexcept
	{
	...
	template <typename T>
	T& ObjectOwner<T>::operator*() const noexcept { return **Pointer; }

	template <typename T>
	T* ObjectOwner<T>::operator->() const noexcept { return (*Pointer).get(); }
